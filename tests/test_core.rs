...
use argus::common::{
    calculate_entropy, find_preceding_identifier, is_harmless_text, is_likely_charset,
};
use argus::scanner::entropy::{
...
fn request_trace_flags_intent_mismatch() {
    ...
    let lines = request_trace_lines(raw, None).unwrap_or_default();
